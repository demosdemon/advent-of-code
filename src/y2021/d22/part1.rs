/*
    ...
    Operating at these extreme ocean depths has overloaded the submarine's reactor; it
    ...
    off; at the start of the reboot process, they are all off. (Could it be an old model
    of a reactor you've seen before?)
    ...
    (the set of all cubes that have coordinates which fall within ranges for x, y,
    and z) and whether to turn all of the cubes in that cuboid on or off.
    ...
    The first step (on x=10..12,y=10..12,z=10..12) turns on a 3x3x3 cuboid consisting of
    27 cubes:
    ...
    overlaps with the first. As a result, only 19 additional cubes turn on; the rest are
    already on from the previous step:
    ...
    The last two steps are fully outside the initialization procedure area; all other
    steps are fully within it. After executing these steps in the initialization
    procedure region, 590784 cubes are on.

    Execute the reboot steps. Afterward, considering only cubes in the region
    x=-50..50,y=-50..50,z=-50..50, how many cubes are on?
*/

const CLAMP: super::Cube = super::Cube::new(
